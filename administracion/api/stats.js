...
/**
 * GET /api/stats
 * Panel de control: agrega la lista del license-server en un payload de dashboard.
 * Calcula métricas que el upstream no da listas (por vencer, activaciones totales,
 * clientes distintos, ingresos potenciales por tipo) para no recalcular en el navegador.
 */
...
module.exports = async function handler(req, res) {
  ...
  for (const l of licenses) {
    ...
    // Por vencer: activas, no permanentes, con días restantes dentro del umbral (>= 0).
    if (
      l.status === 'active' &&
      l.type !== 'permanent' &&
